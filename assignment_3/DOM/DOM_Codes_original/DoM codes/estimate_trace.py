...
for kb in range(256):
    ...
    with open(myfile) as csv_file:
        ...
        for row in csv_reader:
            ...
            if (binexp[0] == '1'):
                sarr_a = np.add(sarr_a, np.array(
                    row[wstart:wstop]).astype(np.float))
                # print sarr_a
                ntraces_a = ntraces_a + 1
            if (binexp[0] == '0'):
                sarr_b = np.add(sarr_b, np.array(
                    row[wstart:wstop]).astype(np.float))
                # print sarr_b
...
for i in range(256):
    ...
    tp = range(len(row))
    if (i == correct_key):
        plt.plot(range(len(correct_row)), correct_row, 'r',
                 linewidth=0.2, label='Correct Key Byte')
    else:
